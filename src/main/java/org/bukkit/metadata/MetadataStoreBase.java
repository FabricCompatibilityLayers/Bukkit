...
public abstract class MetadataStoreBase<T> {
	...
	/**
	 * Caches the results of calls to {@link MetadataStoreBase#disambiguate(Object, String)} in a {@link WeakHashMap}.
	 * Doing so maintains a
	 * <a href="http://www.codeinstructions.com/2008/09/weakhashmap-is-not-cache-understanding.html">canonical list</a>
	 * of disambiguation strings for objects in memory. When those objects are garbage collected, the disambiguation
	 * string
	...
	/**
	 * Creates a unique name for the object receiving metadata by combining unique data from the subject with a
	 * metadataKey.
	 * The name created must be globally unique for the given object and any two equivalent objects must generate the
	 * same unique name. For example, two Player objects must generate the same string if they represent the same
	 * player,
